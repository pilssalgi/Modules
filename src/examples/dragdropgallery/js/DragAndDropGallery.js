...
function DragAndDropGallery($wrap,option){
  ...
  var size 				= {left:0,width:0,start:0,end:0};
  var config 			= {
  ...
  $.extend(config,option);
    ...
    $(window).on('resize',debounce(onResize,30));
    onResize();
	...
	function onResize(){
		...
		size.margin = 0;
		deviation = (($item.innerWidth()+size.margin)*$item.length) - $wrap.width();
		if(!config.reverse){
			size.start = 0;
  ...
  function onRender(){
    ...
    position.ratio = position.x / size.end;
  	...
  	current = Math.floor(position.x/size.width);
    moveSlide(position.x);
  }
  ...
  function translate3d(dom,x,y,z){
	...
	this.prev = function(){
		...
		if(x<size.start)x = size.start;
		if(Tweener)Tweener.kill();
		Tweener = TweenLite.to(position,0.7,{x:x,ease:Power2.easeInOut,onUpdate:function(){
			moveSlide(position.x);
		}});
	}
	...
	this.next = function(){
		current++;
		if(current > $item.length)current = $item.length-1;
		var x = current*size.width;
		if(x>size.end)x = size.end;
		if(Tweener)Tweener.kill();
		Tweener = TweenLite.to(position,0.7,{x:x,ease:Power2.easeInOut,onUpdate:function(){
			moveSlide(position.x);
		}});
