...
function Main(wrap,videoDom,imageUrl,option){
	...
	function render(){
		...
		// uniforms.tMask.value = video;
		// if(isRender)requestAnimationFrame(render);
	}


	var tweens = [];
	var playTime = {currentTime:0};
	var playTweener = null;
	this.play = function(){
		...
		playTweener = TweenLite.to(playTime,delay+11.2,{currentTime:video.duration,ease:Power0.easeNone,onUpdate:function(){
			render();
		}});
	...
	this.reset = function(){
		...
		if(playTweener)playTweener.kill();
